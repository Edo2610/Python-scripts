...
def auc_score(labels, preds, classes):
    label = np.asarray(labels)
    pred = np.asarray(preds)
    
    y_true = np.zeros((len(label),len(classes)), dtype=int)
    y_pred = np.zeros((len(pred),len(classes)), dtype=int)
    ...
    auc.compute_AUC_scores(y_true, y_pred, classes)
